...
use crate::tercen::{
    e_meta_factor, CrosstabSpec, CubeAxisQuery, CubeQuery, EMetaFactor, Factor, MetaFactor,
};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let global_column_df =
        create_global_column_df(&tercen_ctx, &sample_meta_factor, &cube_queries).await?;
    ...
    for (cube_query, drawing_area) in cube_queries.iter().zip(drawing_areas.into_iter()) {
        ...
        .await?;
        let result = draw_cube_query(&global_column_df, &drawing_area, &cube_query, qt_df);
...
fn get_input_spec(cube_query: CubeQuery) -> Result<CrosstabSpec, Box<dyn Error>> {
    ...
    if TercenArgs::try_parse().is_ok() {
        ...
        let input_spec = match input_spec_object {
            ...
            Object::Operatorinputspec(input_spec) => {
...
fn draw_cube_query(
    ...
    let data_frames = qt_df.partition_by(["_sample_offset"], true)?;
...
async fn create_global_column_df(
    tercen_ctx: &TercenContext,
    sample_meta_factor: &MetaFactor,
    cube_queries: &Vec<CubeQuery>,
    ...
    let sample_factor_names = sample_meta_factor
        ...
        .map(|f| f.name.clone())
        .collect::<Vec<_>>();
    let (first_query, rest) = cube_queries
        .split_first()
        .ok_or_else(|| TercenError::new("nothing to compute."))?;

    let mut global_column_df = tercen_ctx
        .select_data_frame_from_id(&first_query.column_hash, sample_factor_names.clone())
        .await?;

    for cube_query in rest {
        let col_df = tercen_ctx
            .select_data_frame_from_id(&cube_query.column_hash, sample_factor_names.clone())
            .await?;
    ...
    global_column_df.insert_column(
        0,
        Series::new("_sample_offset", (0..n_samples).collect::<Vec<_>>()),
    )?;
    Ok(global_column_df)
}
...
async fn create_qt_df(
    ...
    col_df.insert_column(
        0,
        Series::new(".ci", (0..(col_df.shape().0 as i32)).collect::<Vec<_>>()),
    )?;
