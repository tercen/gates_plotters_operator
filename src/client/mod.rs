...
use crate::tercen::{CubeQuery, e_schema, ESchema, GetRequest, Pair, ReqConnect, ReqGetCubeQuery, ReqStreamTable, TableSchema};
...
impl ServiceFactory {
    ...
    pub async fn connect(&mut self, username_or_email: String, password: String) -> Result<(), Box<dyn Error>> {
        let mut client = UserServiceClient::new(self.channel.clone());

        let response = client.connect(ReqConnect {
            username_or_email,
            password,
        }).await?;


        let req_token = response.into_inner()
            .result
    ...
    fn auth_interceptor(&self) -> Result<AuthInterceptor, Box<dyn Error>> {
        let interceptor = match self.token.as_ref() {
            None => AuthInterceptor { authorization: None },
            Some(token) => AuthInterceptor { authorization: Some(token.parse()?) }
        };
    ...
    pub fn user_service(&self) -> Result<UserServiceClient<InterceptedService<Channel, AuthInterceptor>>, Box<dyn Error>> {
        Ok(UserServiceClient::with_interceptor(self.channel.clone(), self.auth_interceptor()?))
    }

    pub fn workflow_service(&self) -> Result<WorkflowServiceClient<InterceptedService<Channel, AuthInterceptor>>, Box<dyn Error>> {
        Ok(WorkflowServiceClient::with_interceptor(self.channel.clone(), self.auth_interceptor()?))
    }

    pub fn table_service(&self) -> Result<TableSchemaServiceClient<InterceptedService<Channel, AuthInterceptor>>, Box<dyn Error>> {
        Ok(TableSchemaServiceClient::with_interceptor(self.channel.clone(), self.auth_interceptor()?))
    }

    pub fn file_service(&self) -> Result<FileServiceClient<InterceptedService<Channel, AuthInterceptor>>, Box<dyn Error>> {
        Ok(FileServiceClient::with_interceptor(self.channel.clone(), self.auth_interceptor()?))
    }

    pub fn task_service(&self) -> Result<TaskServiceClient<InterceptedService<Channel, AuthInterceptor>>, Box<dyn Error>> {
        Ok(TaskServiceClient::with_interceptor(self.channel.clone(), self.auth_interceptor()?))
    }
}
...
impl TercenContext {
    pub async fn new() -> Result<Self, Box<dyn Error>> {
        match TercenArgs::try_parse() {
            Ok(args) => {
                let uri = Uri::from_static(&args.serviceUri);
            ...
            Err(_) => {
                ...
                let mut factory = ServiceFactory::new(uri).await?;
                factory.connect("admin".to_string(), "admin".to_string()).await?;
                Ok(TercenContext { factory })
            }
        }
    }

    pub async fn get_cube_query_from_task(&self, task_id: &str) -> Result<CubeQuery, Box<dyn Error>> {
        let response = self.factory
            .task_service()?
            .get(GetRequest { id: task_id.to_string() })
            .await?;

        let cube_query = response.into_inner().object.and_then(|object| {
            match object {
    ...
    pub async fn get_cube_queries_from_task(&self) -> Result<Vec<CubeQuery>, Box<dyn Error>> {
        if (TercenArgs::try_parse().is_ok()) {
            ...
            Ok(vec![self.get_cube_query().await?, self.get_cube_query().await?])
        }
    }
    ...
    pub async fn get_cube_queries_from_task_ids(&self) -> Result<Vec<String>, Box<dyn Error>> {
        let ids = match TercenArgs::try_parse() {
            Ok(args) => {
                ...
                let task_env = response.into_inner().object.and_then(|object| {
                    match object {
                ...
                match task_env.iter().find(|env| env.key.eq("tercen.gating.task.ids")) {
                    None => {
                        vec![]
                    }
                    Some(env) => {
                        env.value.split(",").map(|e| e.to_string()).collect::<Vec<_>>()
                    }
                }
            }
            Err(_) => {
    ...
    pub async fn get_cube_query(&self) -> Result<CubeQuery, Box<dyn Error>> {
        match TercenArgs::try_parse() {
            Ok(args) => {
                Ok(self.get_cube_query_from_task(&args.taskId).await?)
            }
            Err(_) => {
                ...
                let response = self.factory.workflow_service()?
                    .get_cube_query(ReqGetCubeQuery {
                        workflow_id: workflow_id.clone(),
                        step_id: step_id.clone(),
                    }).await?;

                let cube_query = response.into_inner()
                    .result
    ...
    pub async fn get_schema(&self, id: &str) -> Result<ESchema, Box<dyn Error>> {
        let response = self.factory.table_service()?
            .get(GetRequest { id: id.to_string() }).await?;
    ...
    pub async fn select_data_frame(&self, column_names: Vec<String>) -> Result<DataFrame, Box<dyn Error>> {
        let cube_query = self.get_cube_query().await?;
        Ok(self.select_data_frame_from_id(&cube_query.qt_hash, column_names).await?)
    }


    pub async fn select_data_frame_from_id(&self, id: &str, column_names: Vec<String>) -> Result<DataFrame, Box<dyn Error>> {
        ...
        reader.finish().map_err(|e| Box::new(TercenError::new(&e.to_string())) as Box<dyn Error>)
    }
}
